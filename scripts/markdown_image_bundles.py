"""Shared helpers for packing / unpacking markdown images.tar bundles."""
...
def pack_images_tar(images_dir: Path, tar_path: Path) -> int:
    """Write deterministic uncompressed tar; returns file count."""
    files = list_image_files(images_dir)
    if not files:
    ...
    with tarfile.open(tmp, mode="w") as tar:
        for path in files:
            rel = path.relative_to(images_dir).as_posix()
...
def unpack_images_tar(tar_path: Path, images_dir: Path, *, clean: bool = True) -> int:
    ...
    extract_kwargs: dict = {}
    # Python 3.12+ extraction filter API
    ...
    with tarfile.open(tar_path, mode="r:") as tar:
        ...
        for member in members:
            ...
            # strip accidental images/ prefix from older packs
